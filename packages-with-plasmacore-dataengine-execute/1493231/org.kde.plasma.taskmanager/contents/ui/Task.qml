...
MouseArea {
    ...
    onWheel: {
        ...
        if (wheel.angleDelta.y > 0) {
            ...
            if ( model.IsLauncher    & plasmoid.configuration.requestNewInstance) {
            tasksModel.requestNewInstance(modelIndex());
       ...
       if (!model.IsMaximized) {
            ...
            tasksModel.requestToggleMaximized(modelIndex());
             ...
             if (model.IsActive & plasmoid.configuration.scrollingUpActiveTask === 1)  {
            tasksModel.requestNewInstance(modelIndex());

        }

             if (model.IsActive & plasmoid.configuration.scrollingUpActiveTask === 2) {
         ...
         if ( !model.IsLauncher & !model.IsActive & plasmoid.configuration.restores) {
            tasksModel.requestActivate(modelIndex());
                 ...
                 if (model.IsActive === true & plasmoid.configuration.requestToggleMinimized) {
            tasksModel.requestToggleMinimized(modelIndex());
        } else if (plasmoid.configuration.requestClose )  {
   ...
   ////////////////////////////////////////////////////////////
    ...
    PlasmaCore.FrameSvgItem {
        ...
        property string basePrefix: "minimized"//borealis minimized attention normal
