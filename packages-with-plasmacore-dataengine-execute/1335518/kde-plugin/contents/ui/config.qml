...
Kirigami.FormLayout  {
    ...
    function generate_options_for_generator(generator)
    {
        ...
        for(var variable in options[generator])
        {
            var settings = options[generator][variable]
        ...
        QtControls.Button {
            ...
            onClicked: {
                for(var key in options)
                {
                    ...
                    for(var opt_key in options[key])
                    {
                        var opt = options[key][opt_key]
                        var sane_min = opt["sane_low"]
                        var sane_max = opt["sane_high"]
                        var type = options[key][opt_key]["type"]
