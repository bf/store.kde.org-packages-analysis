...
def reloadTheme():
	# Switch to another theme and back to apply the changes to AmigaBuntu.
	...
	if config['Theme']['name'] == 'AmigaBuntu':
		config['Theme']['name'] = 'breeze-dark'
		config.save()
		time.sleep(1)
	config['Theme']['name'] = 'AmigaBuntu'
	config.save()
...
def setThemeColor(newColor='0,0,0'):
	...
	filename = os.path.abspath(os.path.expanduser('~/.local/share/plasma/desktoptheme/AmigaBuntu/colors'))
	config = KdeConfig(filename)
