            ...
            KQuickAddons.MouseEventListener {
                ...
                onPositionChanged:{
                    ...
                    if( pressed){
                        ...
                        if(kickoffWidth+mouse.x<Screen.width)kickoffWidth =Math.max(kickoffWidth+mouse.x-configureButton.implicitWidth/2,500)
                        
                        if( plasmoid.configuration.bottomHeader){
                            if(kickoffHeight+mouse.y<Screen.height){kickoffHeight =Math.max(kickoffHeight-configureButton.implicitHeight/2+mouse.y,500)};
                        }else{
                            if(kickoffHeight-mouse.y<Screen.height){kickoffHeight =Math.max(kickoffHeight-mouse.y+configureButton.implicitHeight/2,500)};
                        }
